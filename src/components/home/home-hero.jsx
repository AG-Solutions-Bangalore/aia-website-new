...
export default function HomeHero({ slug, bottombar = false }) {
  ...
  if (isLoading) {
    return (
      /*
        The loaded state renders:
        1. A carousel image (aspect-ratio 16/5)
        2. On desktop: a bottombar div that is position:absolute, 
           translate-y-1/2, overlapping the section below by ~half its height.

        The skeleton MUST mirror this structure exactly so the space
        claimed during loading matches the space claimed after load.
        Any difference = layout shift.
      */
      <section className="relative">
        ...
        {bottombar && (
          <>
            {/*
              Desktop: bottombar is position:absolute + translate-y-1/2.
              It overlaps the next section, so the next section needs
              padding-top equal to half the bottombar height.
              The skeleton must reserve this same overlap space so the
              next section doesn't jump when the real bar appears.

              We do this with a transparent spacer div that is only
              visible on lg+ and has the same height as half the bottombar.
              The bottombar is ~88px tall → half = 44px.
            */}
            <div
